...
func newApp(db *gorm.DB, opts ...gen.DOOption) app {
	...
	_app.CreatedAt = field.NewInt64(tableName, "created_at")
	_app.UpdatedAt = field.NewInt64(tableName, "updated_at")
	_app.Attrs = appHasManyAttrs{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Attrs", "model.Attr"),
	}
...
type appHasManyAttrs struct {
	db *gorm.DB

	field.RelationField
}

func (a appHasManyAttrs) Where(conds ...field.Expr) *appHasManyAttrs {
	if len(conds) == 0 {
		return &a
	}
...
func (a appHasManyAttrs) WithContext(ctx context.Context) *appHasManyAttrs {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a appHasManyAttrs) Model(m *model.App) *appHasManyAttrsTx {
	return &appHasManyAttrsTx{a.db.Model(m).Association(a.Name())}
}

type appHasManyAttrsTx struct{ tx *gorm.Association }

func (a appHasManyAttrsTx) Find() (result []*model.Attr, err error) {
	return result, a.tx.Find(&result)
}

func (a appHasManyAttrsTx) Append(values ...*model.Attr) (err error) {
...
func (a appHasManyAttrsTx) Replace(values ...*model.Attr) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a appHasManyAttrsTx) Delete(values ...*model.Attr) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a appHasManyAttrsTx) Clear() error {
	return a.tx.Clear()
}

func (a appHasManyAttrsTx) Count() int64 {
	return a.tx.Count()
}
